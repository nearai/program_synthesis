...
from program_synthesis.naps.examples.seq2seq import executor, bleu, data
...
def compute_bleu(example, res):
    try:
        code_sequence = None
        if hasattr(res, 'code_sequence') and res.code_sequence is not None:
            code_sequence = res.code_sequence
        elif hasattr(res, 'code_tree') and res.code_tree is not None:
            code_sequence = data.tokenize_code_line(uast_to_lisp.uast_to_lisp(res.code_tree))
        else:
            code_sequence = res
        score = bleu.compute_bleu([example['code_sequence']], [code_sequence])
...
def get_stats_from_code(args):
    ...
    if len(example['eval_tests']) == 0:
        return None
    if executor_ is not None:
        stats = executor.evaluate_code(
            res.code_tree if res.code_tree else res.code_sequence, example['eval_tests'],
            executor_)
