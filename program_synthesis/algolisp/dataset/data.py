...
class WordCodeVocab(object):
    ...
    def __init__(self, word_vocab, code_vocab, num_placeholders):
        self._word_vocab = word_vocab
        self._rev_word_vocab = get_rev_vocab(word_vocab)
        self._code_vocab = code_vocab
        self._rev_code_vocab = get_rev_vocab(code_vocab)
        self._num_placeholders = num_placeholders
        self.reset()

    def reset(self):
        self._last_idx = 0
        self._indicies = list(range(0, self._num_placeholders))
        random.shuffle(self._indicies)
        self._placeholders = {}
        self._rev_placeholders = {}
        self._unks = collections.defaultdict(int)

    @property
    def code_vocab_size(self):
        return len(self._code_vocab)
    ...
    def _get_placeholder(self, key, offset):
        if key not in self._placeholders:
            if self._last_idx >= self._num_placeholders:
                self._unks[key] += 1
                return UNK_TOKEN
            ...
            self._rev_placeholders[self._indicies[self._last_idx]] = key
            self._last_idx += 1
        return self._placeholders[key] + offset        

    def wordtoi(self, key):
        if key in self._word_vocab:
            return self._word_vocab[key]
        return self._get_placeholder(key, len(self._word_vocab))

    def codetoi(self, key):
        if key in self._code_vocab:
            return self._code_vocab[key]
        return self._get_placeholder(key, len(self._code_vocab))

    def itoword(self, idx):
        if idx in self._rev_word_vocab:
            return self._rev_word_vocab[idx]
        else:
            return self._rev_placeholders.get(idx - len(self._word_vocab), "PL@%d" % idx)

    def itocode(self, idx):
        if idx in self._rev_code_vocab:
            return self._rev_code_vocab[idx]
        else:
            return self._rev_placeholders.get(idx - len(self._code_vocab), "PL@%d" % idx)
...
class SExprWrapper(object):
    ...
    def _repr_pretty_(self, p, cycle):
        ...
        elif isinstance(self.obj, six.string_types):
            p.text(self.obj)
        else:
            p.pretty(self.obj)
